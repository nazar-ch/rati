...
global request cache ("What rati is not", docs/current/public/guide.md). So a
...
scope-and-island-directions.md) and the prefetch
...
(packages/rati/src/router/scrollRestoration.ts); an async
...
(router-extensions.md §Navigation status names it explicitly),
...
- **Mutation serialization.** TQ's [mutation `scope.id`](https://tanstack.com/query/latest/docs/framework/react/guides/mutations)
  ...
  "mutation coalescing/serialization" question (docs/archive/directions-2026-07/data-package.md §Open questions).
