...
case. If a direction ships, its record moves to docs/archive; if it's actively
being executed, it becomes a docs/planned effort. Anything that stays here is,
...
docs/archive/directions-2026-07; its still-open options were
...
- scope-model-capabilities.md — the IMP-02 walk of what
...
- scope-and-island-directions.md — abort signals for data
...
- router-extensions.md — `group`/`include` composition, nested wrapper
...
- stores-and-router.md — the stores-container pattern assessment: what
  it buys, the router-induced dependency cycles, and the options for resolving them.
- stores-container-implementation.md — the concrete
...
- ssg-and-rsc.md — SSG (a build script over the shipped `renderApp`) and RSC
  as a compatibility constraint; direction only.
- dx-and-tooling.md — test utilities (`rati/testing`), a `dataTrace`
  sibling to `navTrace`, DevTools naming for `Step` components. **In execution:**
  docs/planned/testing-and-dx/README.md (cut 2026-07-19).
