...
> **Resolved 2026-07-26 — implemented, beyond Option A.** The rati side landed in one
> change with a stronger cut: instead of keeping the stores skeleton with a table-blind
> `router` slot, rati dropped the skeleton entirely (`RootStore` / `RootStoreProvider` /
> `GlobalStore` / `GlobalStores` / `GenericStoresContext` / `createUseStoresHook` — all
> removed). What shipped: `createRouter(routes, options): Router` (the class went
> internal), the table-blind **`Router`** interface (this doc's `AppRouter`, name
> resolved), `RouterProvider`, `RouterOutlet` (the outlet, renamed from `Router`),
> `useRouter(): Router`, and `createTestRouter`'s `stores` option replaced by a generic
> `wrapper` (the `storesWrapper`/`renderWithStores` seam moved to app land with the
