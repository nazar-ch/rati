...
> convention (`issues/<ID>-<slug>.md` with a `status:` field —
> $JNANA_KIT_HOME/plugin/docs/issue-tracking.md): its tasks are bullets in one file and its
> "Done" banners are hand-kept. Read them as historical; anything cut out of here from now on
> becomes a record.
