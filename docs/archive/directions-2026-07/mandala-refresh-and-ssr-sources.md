...
`reference.md`, internals in `docs/internals.md`). Still pending: the examples gallery, the `data()`
rename, and a `rati/mobx` convenience for query-backed seeds. This note records the settled design
and maps it onto the code.
...
- **Verbs, not handles.** The hook never exposes the underlying promises/sources — components keep
  receiving strictly resolved state. A second scope-keyed channel (the *controls channel*,
...
- **Re-runs happen in render, not imperatively.** `refresh(key)` marks the cell dirty and triggers a
  bare re-render; the Step re-runs the producer where `prev` naturally lives — with current upstream
  values, including values a cascade swapped in the same pass (levels render top-down).
- **Stale-while-refetch, no Suspense re-entry.** The old value stays rendered while the re-fetch is
  in flight; on a changed settle the cell becomes a *value cell*, so the new value renders
...
- **Dependents come from recorded reads.** Producers run against a `Proxy` over the resolved bag
  ...
  `(bag) => bag.x` styles re-record per run. Dependents can only live in later levels (a level never
  sees siblings), so the fan-out walks levels below only.
...
- **Scope boundaries**: promise loads only. Sources refresh themselves (the data-package division of
  labor), hook loads re-run every render anyway, static entries have no producer — all warn and
  no-op. `refresh()` with no key is the existing retry mechanism (loading slot shows; it composes
  with the future `keepStale`, improvements.md §2).
- **Failure keeps the previous value.** The returned promise *resolves* (fire-and-forget callers
  don't trip unhandled rejections); the failure is logged. Richer error surfacing on the controls is
  future work.
...
- `mandala/resolver.tsx` — the render halves: `buildCell` (hydration short-circuit, seed
  application, server promotion of SSR sources), `processDirtyCells` (re-run + gate + swap), stale
  rendering in the resolve loop, and the source-lifetime rework (below).
...
cleanup. With swaps, a cleanup can't tell a deps-change from an unmount — so the Step keeps entries
its *live* bucket still holds (releasing swap leavers and stale-bucket runs), and the mandala's
unmount effect sweeps whatever is still attached. Ordering invariants hold: the `.provide()` dispose
is a layout cleanup and every layout cleanup flushes before any passive cleanup, so
dispose-before-detach survives; StrictMode's remount still detaches through the sweep before the
fresh run rebuilds.
...
(`firstSettle`, `mandala/ssrSource.ts`: attach + subscribe until non-pending + detach). The trust is
the same one extended to any promise load; render budgets belong to the prerender helper
...
- **`ssr: true` — a loader in source clothing.** Promise semantics end to end: the ready value
  (JSON-safe) dehydrates into the ordinary `data` section; on the client the key short-circuits to
  that value and the producer never runs — no instance, no attach, no double fetch. Marking a
...
- **`ssr: { hydrate, dehydrate? }` — a live source that can be seeded.** The server dehydrates
  `dehydrate(value)` (default: the value) into the new `seeds` wire section; the client creates the
  source as usual, calls `hydrate(data)` *before* attach, and the first snapshot is already ready —
  no pending gap, no double fetch, fully live afterward. This is the shape for instance-yielding
  sources (a query's `source()`): only data travels, the store graph rebuilds the instance.
- **Unmarked sources are untouched**: pending HTML, client resolution — previous behavior, and the
  right call for anything that can't seed.
...
present** — resolving without dehydration would hand the client ready HTML it must re-render into a
pending source, a guaranteed hydration mismatch. `promiseSource(promise, { ssr })` and
...
- `refresh(key)`'s promise settles when the *key* does; its cascade may still be in flight (visible
  via `pending`).
- Hydrated cells carry no read-set until first re-run (their producer never ran) — they respond to
  direct refresh but aren't cascade targets until then.
