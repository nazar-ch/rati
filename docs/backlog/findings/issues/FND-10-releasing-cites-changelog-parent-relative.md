...
Spotted in passing by jnana-kit's findings-close-2 batch (jnana-kit PR #110), measured from a kit pool slot: `check-doc-links.ts` pointed at this checkout reported `docs/current/RELEASING.md:3 ../../CHANGELOG.md ✗ parent-relative`. Filed 2026-07-30 as the consumer follow-up that report named.
...
2. Decide whether to wire the kit's doc-links check into `verify` (a `verify:*`-style step in the `ci` chain, the way jnana runs it) — the sweep that decision implies is small here (few docs, one known violation), and without it this class stays invisible.
